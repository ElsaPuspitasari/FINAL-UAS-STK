...
def load_database():
    ...
    # Load Q&A
    cursor.execute("SELECT question, answer FROM medical_qa ORDER BY id ASC")
    qa_rows = cursor.fetchall()
    
    # pisahkan list-nya
    questions = [row[0] for row in qa_rows]
    answers = [row[1] for row in qa_rows]

    # Load Embeddings
    cursor.execute("SELECT emb FROM embeddings ORDER BY id ASC")
    emb_rows = cursor.fetchall()
    
    # BLOB → numpy array
    embeddings = []
    for r in emb_rows:
        vector = pickle.loads(r[0])    # BLOB → vector float32
    ...
    return embeddings, questions, answers


# Load model & database
model = load_model()
corpus_emb, corpus_qs, corpus_ans = load_database()
...
with st.form(key='medical_query_form'):
    question = st.text_area(
        ...
        height=120,
        placeholder="Contoh: Nyeri dada ringan saat bergerak. Apa penyebabnya?"
    )
...
<div style="text-align:center; margin-bottom: 1rem;">
    <h2 style="color:#1e293b; font-weight:700; margin-bottom:0.5rem;">
        Fitur Utama MediSearch
    </h2>
    <p style="color:#475569; font-size:1rem;">
        Sistem kami dirancang untuk memberikan pengalaman pencarian medis yang relevan, cepat, dan relevan.
    </p>
</div>
""", unsafe_allow_html=True)
